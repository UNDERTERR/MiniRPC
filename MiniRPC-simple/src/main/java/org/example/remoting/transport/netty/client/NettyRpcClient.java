...
public final class NettyRpcClient implements RpcRequestTransport {
    ...
    //构造 NettyRpcClient 时，初始化 Netty 通信管道（Bootstrap）、设置编解码器、心跳机制、服务发现方式、未完成请求缓存等，构建起完整的 RPC 客户端基础设施。
    /*
    NettyRpcClient 初始化：
├── EventLoopGroup（处理网络事件）
├── Bootstrap（连接设置）
│   ├── IdleStateHandler（心跳机制）
│   ├── RpcMessageEncoder / Decoder（编码解码）
│   └── NettyRpcClientHandler（处理响应）
├── ServiceDiscovery（服务发现 - ZooKeeper）
├── UnprocessedRequests（请求响应关联）
└── ChannelProvider（缓存连接）
    ...
    @SneakyThrows
    //@SneakyThrows 让你在代码中不用显式地处理检查型异常（Checked Exception），也不用在方法上加 throws，但异常依然会在运行时抛出。
