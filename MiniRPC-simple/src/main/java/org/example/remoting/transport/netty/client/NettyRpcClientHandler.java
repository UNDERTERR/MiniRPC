...
public class NettyRpcClientHandler extends ChannelInboundHandlerAdapter {
    ...
    /**
     * Read the message transmitted by the server判断消息类型 → 区分心跳 or 正常响应 → 正确处理响应并通知业务层 → 释放资源。
     */
    @Override
        ...
        try {
            ...
            if (msg instanceof RpcMessage) {
                ...
                if (messageType == RpcConstants.HEARTBEAT_RESPONSE_TYPE) {
                    ...
                    //调用 unprocessedRequests.complete()，通过 requestId 唤醒原先等待结果的 CompletableFuture
    ...
    /*
        当客户端在一段时间内没有发送任何数据，Netty 会触发一个 IdleStateEvent 写空闲事件，
        你就在这里向服务端发送一个心跳包（ping），以保持连接不被断开。
    */
    @Override
